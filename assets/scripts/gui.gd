...
var isAnalysisWindowOpen = false
var screenSize
...
func _ready():
	...
	#Category button
	var categoryPopUpBTN = get_node("MainContainer/Seperator/MenuContainer/MenuLiner/BTN_Category").get_popup()
	...
	#Hide Analysis window at startup
	get_node("MainContainer/Seperator/AnalysisWindow").hide()
...
func BTN_handler(name = "null", id = 0, btn_ID = 0):
	...
	#Switch between analysis and task window
	if(name == "BTN_Analysis"): 
		#Show analysis window
		if(isAnalysisWindowOpen == false):
			get_node("MainContainer/Seperator/TaskWindow").hide()
			get_node("MainContainer/Seperator/AnalysisWindow").show()
			isAnalysisWindowOpen = true
			print("Analysis window opened")
		#Show task window
		elif(isAnalysisWindowOpen == true):
			get_node("MainContainer/Seperator/AnalysisWindow").hide()
			get_node("MainContainer/Seperator/TaskWindow").show()
			isAnalysisWindowOpen = false
			print("Analysis window closed")
	
	#Shows help and info about the program
	if(name == "BTN_Help"):
		pass
	...
	#Add a subtask
	if(name == "BTN_AddSubTask"):
		var subTaskInput = LineEdit.new()
		subTaskInput.set_placeholder("Sub task title")
		TaskEditor.get_node("VSplitContainer").add_child(subTaskInput)
		TaskEditor.get_node("VSplitContainer/BTN_AddSubTask").set_disabled(true)
		var subTaskTitle = yield(subTaskInput, "text_entered")
		TaskEditor.tmpSubtaskList.append(subTaskTitle)
		subTaskInput.queue_free()
		var tmpSubtask = Label.new()
		tmpSubtask.set_text(subTaskTitle)
		TaskEditor.get_node("VSplitContainer").add_child(tmpSubtask)
		TaskEditor.get_node("VSplitContainer/BTN_AddSubTask").set_disabled(false)
	...
	if(name == "BTN_Done"):
		...
		tmpTaskInstance.subtaskList = TaskEditor.tmpSubtaskList 
...
func _on_BTN_CategoryPopUP_item_pressed(ID):
	...
	if(ID == 0):
		...
		InputBar.set_placeholder("Name")
		InputBar.set_pos(screenSize / 2)
		InputBar.set_size(defaultInputBarSize)
		add_child(InputBar)
		get_node("MainContainer/Seperator/MenuContainer/MenuLiner/BTN_Category").set_disabled(true)
		...
		InputBar.queue_free()
		get_node("MainContainer/Seperator/MenuContainer/MenuLiner/BTN_Category").set_disabled(false)
		Refresh()
